...
<template>
    <BaseContainer>
        <!-- <BaseHeader /> -->
        <div class="mb-24">
            <img src="/images/error.png" class="max-w-[500px] mx-auto">
            <h2 class="font-semibold text-black text-center text-[36px] mb-6" v-if="error?.statusCode === 404">{{ error?.statusCode }}, Сторінку не знайдено</h2>
            <h2 class="font-semibold text-black text-center text-[36px] mb-6" v-else-if="error?.statusCode === 403">{{ error?.statusCode }}, Доступ заборонено</h2>
            <h2 class="font-semibold text-black text-center text-[36px] mb-6" v-else-if="error?.statusCode === 401">{{ error?.statusCode }}, Необхідна авторизація</h2>
            <h2 class="font-semibold text-black text-center text-[36px] mb-6" v-else-if="error?.statusCode">Помилка {{ error.statusCode }}</h2>
            <h2 class="font-semibold text-black text-center text-[36px] mb-6" v-else>{{ error?.statusCode }}, Внутрішня помилка сервера</h2>
            <p class="text-[18px] text-gray-500 max-w-[540px] mx-auto text-center mb-6">Щось пішло не так. Схоже, що ваш запит не може бути знайдений. Спробуйте ще раз або поверніться пізніше.</p>
            <div class="flex flex-row items-center justify-between max-w-[450px] mx-auto">
                <UButton size="xl" @click="goBack">
                    <img src="/icon/menu-arrow.svg" class="rotate-[180deg]" />
                    <p>Повернутися</p>
                </UButton>
                <UButton variant="outline" size="xl" @click="goToHome">
